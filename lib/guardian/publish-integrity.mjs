...
import { getCurrentSeason, isTopicInSeason, SEASONAL_ONLY_TOPIC_IDS } from "../season-topics.mjs";
...
function auditStructural(root, slug, locale, data, body, phase, bucket) {
  ...
  if (profile === "checklist" && phase === "publish") {
    ...
    const withWhy = (body.match(/\*\*((?:중요한\s*)?이유|Why it matters|Why):?\*\*/gi) ?? []).length;
...
export function verifyPostIntegrity(root, slug, options = {}) {
  ...
  if (topicId && !topicId.startsWith("meta-")) {
    ...
    if (
      topic &&
      SEASONAL_ONLY_TOPIC_IDS.has(topicId) &&
