  ...
  try {
    ...
    return fs
      ...
      .filter((slug) => {
        ...
        return !/^draft:\s*true\b/m.test(raw);
      });
  } catch {
    return [];
  }
...
7. Follow the assigned SECTION VARIANT, not a one-spine template. Always include Related guides internal links to **published** posts only (/en/blog/slug or /ko/blog/slug — no deleted or draft slugs). English primary, Korean faithful translation (not a summary).
   Example published slugs you may link (pick 3–5 relevant): ${publishedHint || "(resolved at runtime on the live site)"}.
