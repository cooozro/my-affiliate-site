...
4. SEO: unique title & description (description 50-160 chars — this is the meta description), semantic ##/### heading tree that matches ${templatePath}, comparison tables where useful, natural keywords only. Meet minimum body length for profile. No keyword stuffing. Bodies are Markdown that render to the site's semantic HTML (do not emit raw HTML tags).
...
9. **Model depth**: for \`head-to-head\` or \`flagship-smartphones\` / \`budget-smartphones\` / \`laptops\`: name **2–3 specific retail models** (generation + SKU-style token, e.g. Galaxy S25, iPhone 16 Pro, Pixel 9) in H2 sections — not generic "Model A/B". Each model section needs strengths, weaknesses, and a scenario verdict.
9b. **Model deep-dive** (\`model-deep-dive\`): when FOCUS MODEL block is present, the article is a single-product editorial review — spec sheet, strengths/weaknesses, who should buy/skip, one rival section max, Final Verdict with buy/wait/skip. Do **not** invent body image markdown URLs — the pipeline injects 1–2 copyright-safe stock product/lifestyle figures with ALT after Design / Spec sections.
